...
public class EdgeDetector {
	...
	public BufferedImage DetectEdge(BufferedImage image) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				int gX = (p02.getGreen() * 3 + p12.getGreen() * 10 + p22.getGreen() * 3)
						- (p00.getGreen() * 3 + p10.getGreen() * 10 + p20.getGreen() * 3);
				int bX = (p02.getBlue() * 3 + p12.getBlue() * 10 + p22.getBlue() * 3)
						- (p00.getBlue() * 3 + p10.getBlue() * 10 + p20.getBlue() * 3);

				int rY = (p20.getRed() * 3 + p21.getRed() * 10 + p22.getRed() * 3)
						- (p00.getRed() * 3 + p01.getRed() * 10 + p02.getRed() * 3);
				int gY = (p20.getGreen() * 3 + p21.getGreen() * 10 + p22.getGreen() * 3)
						- (p00.getGreen() * 3 + p01.getGreen() * 10 + p02.getGreen() * 3);
				int bY = (p20.getBlue() * 3 + p21.getBlue() * 10 + p22.getBlue() * 3)
						- (p00.getBlue() * 3 + p01.getBlue() * 10 + p02.getBlue() * 3);
		...
		for (int i = 1; i < height - 1; i++) {
			for (int j = 1; j < width - 1; j++) {
				if (cannyEdge[i * width + j] != backgroundNumber)
					continue;
					...
					while (!follow.isEmpty()) {
						...
						for (int ny = Math.max(0, top.y - 1); ny <= Math.min(height - 1, top.y + 1); ny++) {
							for (int nx = Math.max(0, top.x - 1); nx <= Math.min(height - 1, top.x + 1); nx++) {
								if (cannyEdge[ny * width + nx] != backgroundNumber)
									continue;
